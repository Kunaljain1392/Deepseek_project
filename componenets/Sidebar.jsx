...
const Sidebar = ({expand, setExpand}) => {
  ...
  const { setSelectedChat, setChats } = useAppContext();
  ...
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute   ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden' } `}>
        <div>
          ...
          <div className={`mt-8 text-white/25 text-sm ${expand ? "block" : "hidden"}`}>
            ...
            {chats.map((chat)=><ChatLabel key={chat._id} name={chat.name} id={chat._id} chat={chat} openMenu={openMenu} setOpenMenu={setOpenMenu} />)} 
          </div>
